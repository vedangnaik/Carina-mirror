...
void Adafruit_PWMServoDriver::setExtClk(uint8_t prescale) {
  ...
  std::cout << "Mode now 0x" << std::hex << read8(PCA9685_MODE1) << std::dec << std::endl;
#endif
}
...
uint16_t Adafruit_PWMServoDriver::getPWM(uint8_t num) {
    ...
    uint16_t off = (read8(channel_base_reg+3) << 8) + read8(channel_base_reg+2);
...
#endif
    uint8_t channel_base_reg = PCA9685_LED0_ON_L + 4 * num;
    write8(channel_base_reg, (uint8_t)(on & 0xFF));     // write least significant 8 bits of ON
    write8(channel_base_reg+1, on >> 8);                // write most significant 8 bits of ON
    write8(channel_base_reg+2, (uint8_t)(off & 0xFF));  // write least significant 8 bits of OFF
    write8(channel_base_reg+3, off >> 8);                // write most significant 8 bits of OFF
}
