...
5. Run `docker run -d -t -p 3390:3390 -p 2222:22 --name carina_dev_img_cont --mount type=bind,source=<absolute path to Carina>,target="/root/Desktop/Carina" carina_dev_img` to start the container. **Note:** The path to Carina must be *absolute*. Otherwise, this may not work. **Note:** The absolute path must include quotes at the beginning and at the end
